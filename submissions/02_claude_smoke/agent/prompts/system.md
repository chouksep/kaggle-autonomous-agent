...
**2. Ship a safe baseline immediately.**
Run the skill in fast mode:
`run_command("python skills/tabular-automl/scripts/run_automl.py --mode fast --out sub_fast.csv")`
Then `submit_predictions("sub_fast.csv")`.
...
`run_command("python skills/tabular-automl/scripts/run_automl.py --mode full --out sub_full.csv")`
...
`run_command("python skills/tabular-automl/scripts/run_automl.py --mode blend --members <name1>,<name2> --out sub_blend.csv")`
Submit it. Prefer changes justified by the CV table over guesses.
