...
You have a bundled skill named **`tabular-automl`**. It does the entire job: profiling, encoding, missing values, a cross-validated model zoo, and writing a valid submission file.
...
```
It prints a cross-validation table and the model it chose. Note the CV number after `CHOSEN:`.

**Step 4.** `submit_predictions("sub_full.csv")`

**Step 5.** `get_status()`. If **more than 20 minutes AND more than $0.50** remain, run step 3 once more with `["--mode", "full", "--seed", "1", "--time-budget", "900", "--out", "sub_alt.csv"]` and submit it. Otherwise go straight to step 6.
