---
name: tabular-automl
description: Complete AutoML pipeline for tabular binary classification. Profiles the data, handles categorical and missing values, fits a cross-validated model zoo (regularized LightGBM, a CV-tuned LightGBM variant on datasets under 20000 rows, HistGradientBoosting, CatBoost, regularized logistic regression), diagnoses a statistically significant weak segment on the winning model, and writes a valid submission file. Always use this instead of writing modelling code by hand.
---
...
| Apply the diagnosed weak-segment recipe | `["--mode", "apply-recipe", "--recipe", "discover_weak_segments", "--time-budget", "900", "--out", "sub_recipe.csv"]` |
...
[info] weak segment diagnosed: gap=0.0934 pval=1.2e-08 n=292
```
...
## Error analysis and the recipe step

Every `--mode full` run also diagnoses whether the winning model has a statistically significant weak segment (an honest-split test, p<0.01) and prints one of:
- `[info] weak segment diagnosed: gap=... pval=... n=...` — a real, worth-trying signal
- `[info] no significant weak segment diagnosed` — nothing to apply, skip the recipe step entirely

Only when the first line appears, `--mode apply-recipe --recipe discover_weak_segments` targets exactly that diagnosed segment (as a new binary feature) and re-runs the zoo, printing `baseline CV ... vs with-recipe CV ...` for direct comparison. It reads the diagnosis from a side file (`error_analysis.json`) written by the preceding `--mode full` run in the same working directory — always run `--mode full` first in a session before ever trying `apply-recipe`.
